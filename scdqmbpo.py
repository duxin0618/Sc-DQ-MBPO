...
class SCDQMBPO(RLAlgorithm):
    ...
    def __init__(
        ...
        super(SCDQMBPO, self).__init__(**kwargs)
    ...
    def _train(self):
        ...
        if self._use_src and self._plan == 1:
            f_log = open(self.log_file, 'a')
            if not self._src_fix_iter:
                info = "plan 1, [{0}, {1}, {2}, {3}] = [x, y, a, b] - my_function_train_iter_number = [{2}, {3}] \n " \
                       "随着 epoch = [{0}, {1}] ," \
                       " min(max((e-x)/(y-x) * (b-a) + a, a), b) \n".format(min_epoch, max_epoch, min_iter, max_iter)
                f_log.write(info)
                print(info)
            else:
                info = "plan {}, fix iters: {} \n".format(self._plan, max_iter)
                f_log.write(info)
                print("plan 1, fix iters: ", max_iter)
            f_log.close()
        elif self._use_src:
            f_log = open(self.log_file, 'a')
            info = "plan {}, fix iters: {} \n".format(self._plan, max_iter)
        ...
        for self._epoch in range(self._epoch, self._n_epochs):
                ...
                if self._timestep % self._model_train_freq == 0:  # 250
                    ...
                    if self._use_src and self._src_epoch_stop > self._epoch \
                        ...
                        model_by_src_function_train_metrics = self._train_model_by_src_function(batch_size=256,
                                                                                                ...
                                                                                                holdout_ratio=0.2,
                                                                                                max_t=self._max_model_t)
                        model_metrics.update(model_by_src_function_train_metrics)
                    ...
                    # if self._use_src:
                    #     self._model.src_copy_saved_source_to_source()
            ...
            # if self._use_src and self._epoch < self._src_epoch_stop:
            #     f_log.write('use src function validation loss: %.8f\n' % model_metrics['src_model_val_loss'])
            #     f_log.write('use src function training loss: %.8f\n' % model_metrics['src_model_train_loss'])
    ...
    def _rollout_by_src_function(self, rollout_batch_size, **kwargs):
        ...
        # Q_info = {}
        for i in range(rollout_src_length):
            real_cur_obs = real_obs[:, i]
            cur_act = act[:, i]
            cur_next_obs = next_obs[:, i]
            cur_rew = reward[:, i]
            cur_term = terminal[:, i]
            ...
            if i > 0:
                # compute Q
                bh = 2e13
                ins = math.floor(len(real_cur_obs)/bh * 1.0)
                src_sample_added_number = 0
                for i in range(ins):
                    real_qs = self._get_Qs_critic_simulated(real_cur_obs[i*bh: i*bh+bh], cur_act[i*bh: i*bh+bh])
                    src_qs = self._get_Qs_critic_simulated(cur_obs[i*bh: i*bh+bh], cur_act[i*bh: i*bh+bh])

                    q_tau = self._reset_Q_tau()
                    q_samples_index = abs(real_qs[0] - src_qs[0]) <= q_tau
                    q_samples_index = q_samples_index.squeeze(-1)
                    samples = {'observations': cur_obs[i*bh: i*bh+bh][q_samples_index], 'actions': cur_act[i*bh: i*bh+bh][q_samples_index], 'next_observations': cur_next_obs[i*bh: i*bh+bh][q_samples_index],
                               'rewards': cur_rew[i*bh: i*bh+bh][q_samples_index],
                               'terminals': cur_term[i*bh: i*bh+bh][q_samples_index]}

                    self._use_src_model_pool.add_samples(samples)

                    src_sample_added_number = src_sample_added_number + len(cur_obs[i*bh: i*bh+bh][q_samples_index])
                steps_added.append(src_sample_added_number)
                print("src_sample_added_number: ", src_sample_added_number)

                # data = {}
                # data['real_Q_1'] = real_qs[0]
                # data['real_Q_2'] = real_qs[1]
                # data['src_Q_1'] = src_qs[0]
                # data['src_Q_2'] = src_qs[1]
                # Q_info['h_{}'.format(i)] = data
        ...
        # if self._epoch % 5 == 0 and (self._src_Q_file_index/250 + 1) % 4 == 0:
            # np.save('./Q_info/{}/Q_epoch_{}_{}_{}'.format(self._env_name, self._epoch, self._src_Q_file_index, int(time.time())), Q_info)
