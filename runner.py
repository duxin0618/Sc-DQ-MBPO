...
class ExperimentRunner:
    ...
    def build(self):
        ...
        # target_file = log_dir+'%s/plan_test_6_%d.config' % (env_name, tag)
        # target_file = log_dir+'%s/%d.config' % (env_name, tag)
        ...
        self.algorithm = get_algorithm_from_variant(
            ...
            log_file=log_dir+'%s/plan_test_6_%d.log' % (self.variant['algorithm_params']['domain'], tag),
            # log_file=log_dir+'%s/%d.log' % (self.variant['algorithm_params']['domain'], tag),
            env_name=env_name,
            tag=tag)
